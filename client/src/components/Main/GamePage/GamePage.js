...
import {SiFandom, SiWikipedia, SiFacebook, SiTwitter, SiTwitch, SiInstagram, SiYoutube, SiApple, SiAndroid, SiSteam, SiReddit, SiEpicgames, SiDiscord} from 'react-icons/si'
...
const GamePage = () => {
    ...
    //Format Screenshot
    const screenshotURL = gameRendered[0]?.artworks[0].url.replace("thumb", "screenshot_big")
        ...
        <div className="Right-app">
          <div className='d-flex flex-column align-items-center'>
            <div className='header w-100'>
                <img width='100%' height='100%' style={{objectFit: 'cover'}} src={screenshotURL} alt={gameRendered[0]?.artworks[0].image_id}/>
            </div>
            <div className='info d-flex flex-column align-items-center '>
                <div className='d-inline-flex align-items-end'>
                    <img className='cover' src={coverURL} alt={gameRendered[0]?.cover.image_id}/>
                ...
                <div className='about w-100 mt-4'>
                            ...
                            <p><span className='fw-bold text-uppercase'>Genre: </span>{gameRendered[0]?.genres[0].name}</p>
                            <p><span className='fw-bold text-uppercase'>Platforms: </span>{platforms}</p>
                                        ...
                                        if(websites_in_game[i] === website.category) {
                                            return(
                                                <div className='d-inline-flex justify-content-center align-items-center mx-2'>
                                                    <a href={gameRendered[0]?.websites[i].url} target='_blank' rel='noreferrer'>
                                                    <button className='d-flex align-items-center' style={{backgroundColor: `${website.color}`, color: 'white', border: 'none'}} >
                                                        {website.icon}
            ...
            <div className='screenshots mt-5'>
                    ...
                    <Row>
                        ...
                        const screenshotURL = screenshot.url.replace("thumb", "screenshot_big")
                        return(
                            <Col className='col-4 d-flex align-items-center justify-content-center my-3'>
                                <img className='screenshot_image' src={screenshotURL} alt={screenshot.image_id} />
                            </Col>
                        )
}                       )}
                    </Row>
